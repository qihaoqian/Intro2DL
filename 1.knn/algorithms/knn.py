...
class KNN(object):
    ...
    def predict(self, x_test: np.ndarray, k: int = None, loop_count: int = 1):
        ...
        neighbors_idx = np.argsort(distance, axis=1)[:, :k_test]  
        neighbors_labels = self._y_train[neighbors_idx]          

        for i in range(num_test):
            votes = np.bincount(neighbors_labels[i])
    ...
    def calc_dis_two_loop(self, x_test: np.ndarray):
        ...
        for i in range(num_test):
            for j in range(num_train):
                diff = x_test[i] - self._x_train[j]
                distance[i, j] = np.sqrt(np.sum(diff**2))
